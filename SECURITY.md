...
1. Malformed or adversarially crafted DICOM files fed to `anonymize.py`.
...
- **Arbitrary code execution**: a crafted DICOM file causes execution of unintended
  code (this would be a pydicom vulnerability; report to pydicom first and CC us).
...
Please report security issues by email to the address listed in the repository
(or the GitHub Security Advisories tab if available).
...
We aim to respond within 5 business days and to issue a patch within 14 days for
confirmed PHI-disclosure issues.

## No bug bounty

This is an open-source project with no commercial entity behind it. We cannot offer
monetary rewards, but we will credit responsible disclosers in the CHANGELOG.
